...
def read_data_file(path):
    ...
    with open(path, 'r') as file:
        for line in file.readlines():
            ...
            inputs_read.append(read_orders(whole_object))
            outputs_read.append([whole_object[2]])

    inputs_read = list(map(normalize_row, inputs_read))
...
def balance_data(input_data, output_data):
    indexes = list(range(0, len(output_data)))
    shuffle(indexes)
    ...
    chosen_indexes = indexes_won + indexes_lost
    shuffle(chosen_indexes)
